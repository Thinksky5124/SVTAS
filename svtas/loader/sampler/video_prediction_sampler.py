...
LastEditTime : 2022-11-07 23:38:57
...
class VideoPredictionFeatureStreamSampler(FeatureStreamSampler):
    ...
    def _sample_pred_label(self, results, sample_rate, pred_sample_num, sample_num, sliding_windows, add_key='pred_labels', sample_key='raw_labels'):
        ...
        if start_frame < frames_len:
            ...
            pred_labels = self._labels_sample(container, start_frame=start_frame, end_frame=end_frame, samples_idx=pred_label_frames_idx).copy()
    ...
    def __call__(self, results):
        ...
        return:
            sampling id.
        """
        results = self._sample_frames(results, self.sample_rate, self.feature_dim, self.clip_seg_num, self.sliding_window, add_key='imgs', sample_key='frames')
        ...
        results = self._sample_pred_label(results, self.sample_rate, self.pred_clip_seg_num, self.clip_seg_num, self.sliding_window, add_key='pred_labels', sample_key='raw_labels')

...
LastEditTime : 2022-12-04 20:56:12
...
@DATASET.register()
class FeatureClipSegmentationDataset(FeatureSegmentationDataset):
    def __init__(self, feature_path, flow_feature_path=None, **kwargs) -> None:
        super().__init__(feature_path, flow_feature_path, **kwargs)
    ...
    def __getitem__(self, index):
        ...
        output_data_dict['feature'] = copy.deepcopy(torch.stack(data_dict['feature'], dim=0))
        output_data_dict['labels'] = copy.deepcopy(np.stack(data_dict['labels'], axis=0))
        output_data_dict['masks'] = copy.deepcopy(np.stack(data_dict['masks'], axis=0))

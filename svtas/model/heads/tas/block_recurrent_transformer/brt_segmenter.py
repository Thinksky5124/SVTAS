...
LastEditTime : 2023-02-28 16:50:36
...
class RecurrentHierarchicalAttentionLayer(nn.Module):
    def __init__(self,
        ...
        # self.att = MultiHeadChunkAttentionLayer(embed_dim=dim, num_heads=num_head, chunck_size=dilation, dropout=0.0, position_encoding=False)
        self.att = AttLayer(q_dim=dim, k_dim=dim, v_dim=dim, r1=2, r2=2, r3=2, bl=dilation, stage='encoder', att_type="block_att")
        self.state = None

    def _clear_memory_buffer(self):
        self.state = None
    
    def forward(self, feature, d_feature, mask=None):
        # feature, state = self.att(feature, state=state)
        # self.state = state.detach()
        feature = self.att(feature, d_feature, mask)
        return feature * mask

class AttModule(nn.Module):
    def __init__(self, dilation, in_channels, out_channels, dim_head=128, state_len=512, num_head=1, stage='encoder', causal=False):
        super(AttModule, self).__init__()
        self.stage = stage
        self.feed_forward = ConvFeedForward(dilation, in_channels, out_channels)
        self.instance_norm = nn.InstanceNorm1d(in_channels, track_running_stats=False)
        ...
        #                                                         dilation= dilation,
        #                                                         causal = causal)
        self.att_layer = AttLayer(in_channels, in_channels, out_channels, 2, 2, 2, dilation, att_type="sliding_att", stage=stage)
        self.conv_1x1 = nn.Conv1d(out_channels, in_channels, 1)
        self.dropout = nn.Dropout()
        if self.stage == "encoder":
            self.alpha = 1.0
        else:
            self.alpha = exponential_descrease(math.sqrt(dilation))
...
class Encoder(nn.Module):
    def __init__(self, num_layers, num_f_maps, input_dim, num_classes, channel_masking_rate):
        ...
        self.layers = nn.ModuleList(
            [AttModule(2 ** i, num_f_maps, num_f_maps, 'encoder') for i in
             range(num_layers)])
...
class Decoder(nn.Module):
    def __init__(self, num_layers, num_f_maps, input_dim, num_classes):
        ...
        self.layers = nn.ModuleList(
            [AttModule(2 ** i, num_f_maps, num_f_maps, 'decoder') for i in # 2 ** i
             range(num_layers)])
        self.conv_out = nn.Conv1d(num_f_maps, num_classes, 1)
...
class BRTSegmentationHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoders = nn.ModuleList([copy.deepcopy(Decoder(num_layers, num_f_maps, num_classes, num_classes)) for s in range(num_decoders)]) # num_decoders

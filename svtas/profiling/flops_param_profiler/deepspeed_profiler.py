'''
Author       : Thyssen Wen
Date         : 2023-10-18 11:37:15
LastEditors  : Thyssen Wen
LastEditTime : 2023-10-18 21:11:56
Description  : ref: https://github.com/microsoft/DeepSpeed/blob/master/deepspeed/profiling/flops_profiler/profiler.py
FilePath     : /SVTAS/svtas/profiling/flops_param_profiler/deepspeed_profiler.py
'''
...
from svtas.dist import get_world_size_from_os
...
class DeepspeedProfiler(BaseProfiler):
    ...
    The SVTAS flops profiler can be used with the SVTAS runtime or as a standalone package.
    When using SVTAS for model training, the flops profiler can be configured in the deepspeed_config file and no user code change is required.
    ...
    """
    def __init__(self, profile_step: int = 1, recompute_fwd_factor=0.0) -> None:
    ...
    def print_model_profile(self, profile_step=1, module_depth=-1, top_modules=1, detailed=True, output_file=None):
        ...
        logger.log(line_fmt.format('world size: ', get_world_size_from_os()))
        logger.log(line_fmt.format('data parallel size: ', get_world_size_from_os()))

        logger.log(line_fmt.format('params per GPU: ', params_to_string(total_params)))
        if total_model_expert_params > 0:
            ...
            logger.log(line_fmt.format('   non-expert params of model: ', params_to_string(total_model_nonexpert_params)))
            logger.log(line_fmt.format('   expert params of model: ', params_to_string(total_model_expert_params)))
        else:
            logger.log(
                line_fmt.format('params of model = params per GPU * mp_size: ',
                                params_to_string(total_model_nonexpert_params)))

        logger.log(line_fmt.format('fwd MACs per GPU: ', macs_to_string(total_macs)))

        logger.log(line_fmt.format('fwd flops per GPU: ', number_to_string(total_flops)))

        fwd_latency = self.get_total_duration()
        logger.log(line_fmt.format('fwd latency: ', duration_to_string(fwd_latency)))
        logger.log(
        ...
        if detailed:
            ...
            logger.log(
                "\nNote: 1. A module can have torch.nn.module or torch.nn.functional to compute logits (e.g. CrossEntropyLoss). They are not counted as submodules, thus not to be printed out. However they make up the difference between a parent's MACs (or latency) and the sum of its submodules'.\n2. Number of floating-point operations is a theoretical estimation, thus FLOPS computed using that could be larger than the maximum system throughput.\n3. The fwd latency listed in the top module's profile is directly captured at the module forward function in PyTorch, thus it's less than the fwd latency shown above which is captured in SVTAS.\n"
            )
    ...
    def print_model_aggregated_profile(self, module_depth=-1, top_modules=1):
        ...
        if not hasattr(self.model, "__flops__"):
            logger.log("no __flops__ attribute in the model, call this function after start_profile and before end_profile")
            return
        ...
        logger.log(f'Top {top_modules} modules in terms of params, MACs or fwd latency at different model depths:')
        ...
        for d in range(depth):
            ...
            logger.log(f"depth {d}:")
            logger.log(f"    params      - {sort_params}")
            logger.log(f"    MACs        - {sort_macs}")
            logger.log(f"    fwd latency - {sort_time}")
...
def _upsample_flops_compute(*args, **kwargs):
    ...
    elif isinstance(scale_factor, (tuple, list)) and len(scale_factor) > 1:
        final_list = [1 for _ in range(len(input) - len(scale_factor))] + scale_factor
        scale_factor = tuple(final_list)
        flops *= int(_prod(scale_factor))
    else:
